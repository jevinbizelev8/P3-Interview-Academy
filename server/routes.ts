...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Enhanced Perform module API routes
  app.post("/api/perform/simulation/generate", async (req, res) => {
    try {
      const { jobRole, companyName, questionCount = 5, difficultyLevel = 3, questionTypes = ['behavioral', 'situational'] } = req.body;
      
      if (!jobRole || !companyName) {
        return res.status(400).json({ message: "Job role and company name are required" });
      }

      const { performService } = await import("./services/perform-service");
      const questions = await performService.generateSimulationQuestions({
        jobRole,
        companyName,
        questionCount,
        difficultyLevel,
      ...
      res.status(500).json({ message: "Failed to generate simulation questions" });
    }
  });

  app.post("/api/perform/assessment", async (req, res) => {
    try {
      const { sessionId, userId, jobRole, companyName } = req.body;
      ...
      const { performService } = await import("./services/perform-service");
      const assessment = await performService.createPerformanceAssessment(sessionId, userId, jobRole, companyName);
      
      res.json(assessment);
    } catch (error) {
      console.error("Error creating performance assessment:", error);
      res.status(500).json({ message: "Failed to create assessment" });
    }
  });

  app.get("/api/perform/assessments/user/:userId", async (req, res) => {
    try {
      const { userId } = req.params;
      const limit = parseInt(req.query.limit as string) || 10;

      const { performService } = await import("./services/perform-service");
      const assessments = await performService.getUserAssessmentHistory(userId, limit);
  ...
  app.get("/api/perform/overview/user/:userId", async (req, res) => {
    try {
      const { userId } = req.params;

      const { performService } = await import("./services/perform-service");
      const overview = await performService.getUserPerformanceOverview(userId);
  ...
  app.get("/api/perform/drills/user/:userId", async (req, res) => {
    try {
      const { userId } = req.params;

      const { performService } = await import("./services/perform-service");
      const drills = await performService.getUserLearningDrills(userId);
      
      res.json(drills);
    } catch (error) {
      console.error("Error fetching learning drills:", error);
      res.status(500).json({ message: "Failed to fetch learning drills" });
    }
  });

  app.post("/api/perform/drills/:drillId/complete", async (req, res) => {
    try {
      ...
      const { performService } = await import("./services/perform-service");
      const drill = await performService.completeLearningDrill(drillId, userId);
      
      res.json(drill);
    } catch (error) {
      console.error("Error completing drill:", error);
      res.status(500).json({ message: "Failed to complete drill" });
    }
  });

  app.get("/api/perform/indicators", async (req, res) => {
    try {
      const { PERFORMANCE_INDICATORS, PERFORMANCE_RATINGS } = await import("./services/perform-service");
      res.json({ indicators: PERFORMANCE_INDICATORS, ratings: PERFORMANCE_RATINGS });
    } catch (error) {
