...
interface ExecutivePresenceBuilderProps {
  moduleId?: string;
  onComplete?: (score: number, userData: ExecutivePresenceData) => void;
}

interface ExecutivePresenceData {
...
export default function ExecutivePresenceBuilder({
  ...
  const presenceElements: PresenceElement[] = [
  ...
  const saveProgressMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('/api/prepare/modules/progress', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          moduleId: moduleId || 'executive-presence-builder',
